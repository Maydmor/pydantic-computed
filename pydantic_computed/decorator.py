...
def get_parameters(method, values, property_key):
    ...
    parameters = {
        ...
        for property in parameter_names
    ...
    new_kwargs = {
        property: values[property]
        for property in new_kwarg_names
        if property != property_key
    }

    # pass in var keyword arguments if function has e.g. **kwargs in signature
    def contains_var_keyword_arg(params):
        return (
            len(list(filter(lambda param: param.kind == param.VAR_KEYWORD, params))) > 0
        )

    method_args = (
        dict(new_kwargs, **parameters)
        if contains_var_keyword_arg(method_params.values())
        else parameters
    )
    return method_args
...
def computed(property_key: str):
    def __wrapper(method: classmethod):
        ...
        def compute(cls, values):
            ...
            for prop_key, computed_method in COMPUTED_FIELDS[
                qualified_class_name
            ].items():
                parameters = get_parameters(computed_method, values, prop_key)
                values[prop_key] = computed_method(**parameters)
            values[property_key] = COMPUTED_FIELDS[qualified_class_name][property_key](
                **values
            )
            return values
        ...
        return root_validator(allow_reuse=True)(compute)  # type: ignore
